...
public class Grid : MonoBehaviour {
	...
	//Internal variables
	private Dictionary<string, Tile1> grid = new Dictionary<string, Tile1>();
	...
	public Dictionary<string, Tile1> Tiles {
		get {return grid;}
	}
	#endregion
	...
	public Tile1 TileAt(CubeIndex index){
		if(grid.ContainsKey(index.ToString()))
		   return grid[index.ToString()];
		return null;
	}

	public Tile1 TileAt(int x, int y, int z){
		return TileAt(new CubeIndex(x,y,z));
	}

	public Tile1 TileAt(int x, int z){
		return TileAt(new CubeIndex(x,z));
	}

	public List<Tile1> Neighbours(Tile1 tile) {
		List<Tile1> ret = new List<Tile1>();
		CubeIndex o;
	...
	public List<Tile1> Neighbours(CubeIndex index){
		return Neighbours(TileAt(index));
	}

	public List<Tile1> Neighbours(int x, int y, int z){
		return Neighbours(TileAt(x,y,z));
	}

	public List<Tile1> Neighbours(int x, int z){
		return Neighbours(TileAt(x,z));
	}

	public List<Tile1> TilesInRange(Tile1 center, int range){
	...
	public List<Tile1> TilesInRange(CubeIndex index, int range){
		return TilesInRange(TileAt(index), range);
	}

	public List<Tile1> TilesInRange(int x, int y, int z, int range){
		return TilesInRange(TileAt(x,y,z), range);
	}

	public List<Tile1> TilesInRange(int x, int z, int range){
	...
	public int Distance(Tile1 a, Tile1 b){
	...
	private void GetMesh() {
		hexMesh = null;
		Tile1.GetHexMesh(hexRadius, hexOrientation, ref hexMesh);
	}
	...
	private Tile1 CreateHexGO(Vector3 postion, string name) {
		GameObject go = new GameObject(name, typeof(MeshFilter), typeof(MeshRenderer), typeof(Tile1));
		...
		Tile1 tile = go.GetComponent<Tile1>();
		...
		if(drawOutlines) {
			...
			for(int vert = 0; vert <= 6; vert++)
				lines.SetPosition(vert, Tile1.Corner(tile.transform.position, hexRadius, vert, hexOrientation));
		}
